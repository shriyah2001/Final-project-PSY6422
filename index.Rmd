...
cols <- c('source','themes', 'demographics')
...
fig1 <- ggplot(theme_counts, aes(x = reorder(Themes,-Count), y = Count, text = hover_text)) +
    ...
    labs(x = 'Themes', y = 'Count', title = "THEMES OF THE TOP 1000 ANIME IN 2023", caption = 'Source:Top 1000 anime titles from 2023') +
...
#### Anime is classified according to its demographic, or aimed audience. Typical demographics consist of:
...
+ Kids: Targeted towards Yonger auidence
...
fig2 <- ggplot(dem_counts, aes(x =reorder(Demographics,Count), y = Count, fill = Demographics,text= hover_text)) +
    scale_y_continuous(breaks = seq(0, max(dem_counts$Count), by = 50)) + # To make the intervals on Y axis 100
    geom_bar(stat= 'identity')+
    labs(x= 'Demographics', y= 'Count', title = 'The Demographics of the anime',caption = 'Source:Top 1000 anime titles from 2023') +
...
fig2
# Saving the figure in the figures folder
ggsave(here('Figures', 'Demographics.png'))
```
