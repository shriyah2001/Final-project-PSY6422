...
fig1 <- ggplot(theme_counts, aes(x = reorder(Themes,-Count), y = Count, text = hover_text)) +
    ...
    labs(x = "Themes", y = "Count", title = "BAR GRAPH OF THE THEMES") + # defining labels
    ...
    theme(
        ...
        plot.title = element_text(color = 'skyblue', size = 18, hjust = 0.5),  # Plot title colour set to blue & size was adjusted 
...
> We can better understand which themes and tropes appeal to particular audiences by using demographic data.Therefore we will take a look at that factor as well.Typical demographics consist of:
...
fig2 <- ggplot(dem_counts, aes(x =reorder(Demographics,Count), y = Count, fill = Demographics,text= hover_text)) +
    scale_y_continuous(breaks = seq(0, max(dem_counts$Count), by = 50)) + # To make the intervals on Y axis 50
